...
#include "Player/<PROJECT_NAME>PlayerState.h" //@TODO: For the fname
...
EDataValidationResult UGameFeatureAction_AddAbilities::IsDataValid(TArray<FText>& ValidationErrors)
{
	...
	for (const FGameFeatureAbilitiesEntry& Entry : AbilitiesList)
	{
		...
		for (const F<PROJECT_NAME>AbilityGrant& Ability : Entry.GrantedAbilities)
		{
		...
		for (const F<PROJECT_NAME>AttributeSetGrant& Attributes : Entry.GrantedAttributes)
		{
		...
		for (const TSoftObjectPtr<const U<PROJECT_NAME>AbilitySet>& AttributeSetPtr : Entry.GrantedAbilitySets)
		{
...
void UGameFeatureAction_AddAbilities::HandleActorExtension(AActor* Actor, FName EventName, int32 EntryIndex, FGameFeatureStateChangeContext ChangeContext)
{
	...
	if (AbilitiesList.IsValidIndex(EntryIndex) && ActiveData)
	{
		...
		else if ((EventName == UGameFrameworkComponentManager::NAME_ExtensionAdded) || (EventName == A<PROJECT_NAME>PlayerState::NAME_<PROJECT_NAME>AbilityReady))
		{
...
void UGameFeatureAction_AddAbilities::AddActorAbilities(AActor* Actor, const FGameFeatureAbilitiesEntry& AbilitiesEntry, FPerContextData& ActiveData)
{
	...
	if (UAbilitySystemComponent* AbilitySystemComponent = FindOrAddComponentForActor<UAbilitySystemComponent>(Actor, AbilitiesEntry, ActiveData))
	{
		...
		for (const F<PROJECT_NAME>AbilityGrant& Ability : AbilitiesEntry.GrantedAbilities)
		{
			if (!Ability.AbilityType.IsNull())
			{
		...
		for (const F<PROJECT_NAME>AttributeSetGrant& Attributes : AbilitiesEntry.GrantedAttributes)
		{
			if (!Attributes.AttributeSetType.IsNull())
			{
		...
		U<PROJECT_NAME>AbilitySystemComponent* <PROJECT_NAME>ASC = CastChecked<U<PROJECT_NAME>AbilitySystemComponent>(AbilitySystemComponent);
		for (const TSoftObjectPtr<const U<PROJECT_NAME>AbilitySet>& SetPtr : AbilitiesEntry.GrantedAbilitySets)
		{
			if (const U<PROJECT_NAME>AbilitySet* Set = SetPtr.Get())
			{
				Set->GiveToAbilitySystem(<PROJECT_NAME>ASC, &AddedExtensions.AbilitySetHandles.AddDefaulted_GetRef());
			}
		}
...
void UGameFeatureAction_AddAbilities::RemoveActorAbilities(AActor* Actor, FPerContextData& ActiveData)
{
	if (FActorExtensions* ActorExtensions = ActiveData.ActiveExtensions.Find(Actor))
	{
		if (UAbilitySystemComponent* AbilitySystemComponent = Actor->FindComponentByClass<UAbilitySystemComponent>())
		{
			...
			U<PROJECT_NAME>AbilitySystemComponent* <PROJECT_NAME>ASC = CastChecked<U<PROJECT_NAME>AbilitySystemComponent>(AbilitySystemComponent);
			for (F<PROJECT_NAME>AbilitySet_GrantedHandles& SetHandle : ActorExtensions->AbilitySetHandles)
			{
				SetHandle.TakeFromAbilitySystem(<PROJECT_NAME>ASC);
			}
		}
