...
#include "Character/<PROJECT_NAME>HeroComponent.h"
...
EDataValidationResult UGameFeatureAction_AddInputBinding::IsDataValid(TArray<FText>& ValidationErrors)
{
	...
	for (const TSoftObjectPtr<const U<PROJECT_NAME>InputConfig>& Entry : InputConfigs)
	{
		if (Entry.IsNull())
		{
...
void UGameFeatureAction_AddInputBinding::HandlePawnExtension(AActor* Actor, FName EventName, FGameFeatureStateChangeContext ChangeContext)
{
	...
	else if ((EventName == UGameFrameworkComponentManager::NAME_ExtensionAdded) || (EventName == U<PROJECT_NAME>HeroComponent::NAME_BindInputsNow))
	{
...
void UGameFeatureAction_AddInputBinding::AddInputMappingForPlayer(APawn* Pawn, FPerContextData& ActiveData)
{
	...
	if (ULocalPlayer* LocalPlayer = PlayerController ? PlayerController->GetLocalPlayer() : nullptr)
	{
		...
		{
			U<PROJECT_NAME>HeroComponent* HeroComponent = Pawn->FindComponentByClass<U<PROJECT_NAME>HeroComponent>();
			if (HeroComponent && HeroComponent->IsReadyToBindInputs())
			{
				for (const TSoftObjectPtr<const U<PROJECT_NAME>InputConfig>& Entry : InputConfigs)
				{
					if (const U<PROJECT_NAME>InputConfig* BindSet = Entry.Get())
					{
...
void UGameFeatureAction_AddInputBinding::RemoveInputMapping(APawn* Pawn, FPerContextData& ActiveData)
{
	...
	if (ULocalPlayer* LocalPlayer = PlayerController ? PlayerController->GetLocalPlayer() : nullptr)
	{
		if (UEnhancedInputLocalPlayerSubsystem* InputSystem = LocalPlayer->GetSubsystem<UEnhancedInputLocalPlayerSubsystem>())
		{
			if (U<PROJECT_NAME>HeroComponent* HeroComponent = Pawn->FindComponentByClass<U<PROJECT_NAME>HeroComponent>())
			{
				for (const TSoftObjectPtr<const U<PROJECT_NAME>InputConfig>& Entry : InputConfigs)
				{
					if (const U<PROJECT_NAME>InputConfig* InputConfig = Entry.Get())
					{
