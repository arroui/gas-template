...
#define LOCTEXT_NAMESPACE "<PROJECT_NAME>GameFeatures"
...
{
	for (const F<PROJECT_NAME>HUDElementEntry& Entry : Widgets)
	{
...
EDataValidationResult UGameFeatureAction_AddWidgets::IsDataValid(TArray<FText>& ValidationErrors)
{
	...
	{
		int32 EntryIndex = 0;
		for (const F<PROJECT_NAME>HUDLayoutRequest& Entry : Layout)
		{
...
void UGameFeatureAction_AddWidgets::AddToWorld(const FWorldContext& WorldContext, const FGameFeatureStateChangeContext& ChangeContext)
{
	...
	if ((GameInstance != nullptr) && (World != nullptr) && World->IsGameWorld())
	{
		...
		{			
			TSoftClassPtr<AActor> HUDActorClass = A<PROJECT_NAME>HUD::StaticClass();
...
{
	A<PROJECT_NAME>HUD* HUD = CastChecked<A<PROJECT_NAME>HUD>(Actor);
