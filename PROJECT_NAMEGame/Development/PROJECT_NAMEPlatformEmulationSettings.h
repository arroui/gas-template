...
#include "<PROJECT_NAME>PlatformEmulationSettings.generated.h"

/**
 * Platform emulation settings
 */
UCLASS(config=EditorPerProjectUserSettings, MinimalAPI)
class U<PROJECT_NAME>PlatformEmulationSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	U<PROJECT_NAME>PlatformEmulationSettings();
...
private:
	...
	// The base device profile to pretend we are using when emulating device-specific device profiles applied from U<PROJECT_NAME>SettingsLocal
	...
	UPROPERTY(EditAnywhere, config, Category=PlatformEmulation, meta=(ConsoleVariable="<PROJECT_NAME>.Settings.ApplyFrameRateSettingsInPIE"))
	...
	UPROPERTY(EditAnywhere, config, Category=PlatformEmulation, meta=(ConsoleVariable="<PROJECT_NAME>.Settings.ApplyFrontEndPerformanceOptionsInPIE"))
	bool bApplyFrontEndPerformanceOptionsInPIE = false;

	// Should we apply experience/platform emulated device profiles in PIE?
	UPROPERTY(EditAnywhere, config, Category=PlatformEmulation, meta=(InlineEditConditionToggle, ConsoleVariable="<PROJECT_NAME>.Settings.ApplyDeviceProfilesInPIE"))
