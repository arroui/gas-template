...
F<PROJECT_NAME>GameplayTags F<PROJECT_NAME>GameplayTags::GameplayTags;

void F<PROJECT_NAME>GameplayTags::InitializeNativeTags()
{
...
void F<PROJECT_NAME>GameplayTags::AddAllTags(UGameplayTagsManager& Manager)
{
...
void F<PROJECT_NAME>GameplayTags::AddTag(FGameplayTag& OutTag, const ANSICHAR* TagName, const ANSICHAR* TagComment)
{
	OutTag = UGameplayTagsManager::Get().AddNativeGameplayTag(FName(TagName), FString(TEXT("(Native) ")) + FString(TagComment));
}

void F<PROJECT_NAME>GameplayTags::AddMovementModeTag(FGameplayTag& OutTag, const ANSICHAR* TagName, uint8 MovementMode)
{
	AddTag(OutTag, TagName, "Character movement mode tag.");
	GameplayTags.MovementModeTagMap.Add(MovementMode, OutTag);
}

void F<PROJECT_NAME>GameplayTags::AddCustomMovementModeTag(FGameplayTag& OutTag, const ANSICHAR* TagName, uint8 CustomMovementMode)
{
...
FGameplayTag F<PROJECT_NAME>GameplayTags::FindTagByString(FString TagString, bool bMatchPartialString)
{
	...
	if (!Tag.IsValid() && bMatchPartialString)
	{
		...
		for (const FGameplayTag TestTag : AllTags)
		{
			...
			{
				UE_LOG(Log<PROJECT_NAME>, Display, TEXT("Could not find exact match for tag [%s] but found partial match on tag [%s]."), *TagString, *TestTag.ToString());
				Tag = TestTag;
				break;
			}
		}
