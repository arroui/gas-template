...
#include "<PROJECT_NAME>BotCreationComponent.h"
...
U<PROJECT_NAME>BotCreationComponent::U<PROJECT_NAME>BotCreationComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void U<PROJECT_NAME>BotCreationComponent::BeginPlay()
{
	Super::BeginPlay();
	...
	ExperienceComponent->CallOrRegister_OnExperienceLoaded_LowPriority(FOn<PROJECT_NAME>ExperienceLoaded::FDelegate::CreateUObject(this, &ThisClass::OnExperienceLoaded));
}

void U<PROJECT_NAME>BotCreationComponent::OnExperienceLoaded(const U<PROJECT_NAME>ExperienceDefinition* Experience)
{
#if WITH_SERVER_CODE
	if (HasAuthority())
	{
...
void U<PROJECT_NAME>BotCreationComponent::ServerCreateBots()
{
	...
	if (GIsEditor)
	{
		const U<PROJECT_NAME>DeveloperSettings* DeveloperSettings = GetDefault<U<PROJECT_NAME>DeveloperSettings>();
...
FString U<PROJECT_NAME>BotCreationComponent::CreateBotName(int32 PlayerIndex)
{
	FString Result;
...
void U<PROJECT_NAME>BotCreationComponent::SpawnOneBot()
{
	...
	{
		A<PROJECT_NAME>GameMode* GameMode = GetGameMode<A<PROJECT_NAME>GameMode>();
		check(GameMode);
		...
		{
			if (U<PROJECT_NAME>PawnExtensionComponent* PawnExtComponent = NewController->GetPawn()->FindComponentByClass<U<PROJECT_NAME>PawnExtensionComponent>())
			{
...
void U<PROJECT_NAME>BotCreationComponent::RemoveOneBot()
{
	if (SpawnedBotList.Num() > 0)
	{
		...
		if (BotToRemove)
		{
			...
			{
				if (U<PROJECT_NAME>HealthComponent* HealthComponent = U<PROJECT_NAME>HealthComponent::FindHealthComponent(ControlledPawn))
				{
