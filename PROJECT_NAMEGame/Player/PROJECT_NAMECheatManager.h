...
#include "<PROJECT_NAME>CheatManager.generated.h"
...
DECLARE_LOG_CATEGORY_EXTERN(Log<PROJECT_NAME>Cheat, Log, All);


/**
 * U<PROJECT_NAME>CheatManager
 *
 *	Base cheat manager class used by this project.
 */
UCLASS(config = Game, Within = PlayerController, MinimalAPI)
class U<PROJECT_NAME>CheatManager : public UCheatManager
{
	GENERATED_BODY()
...
protected:
	...
	void ApplySetByCallerDamage(U<PROJECT_NAME>AbilitySystemComponent* <PROJECT_NAME>ASC, float DamageAmount);
	void ApplySetByCallerHeal(U<PROJECT_NAME>AbilitySystemComponent* <PROJECT_NAME>ASC, float HealAmount);

	U<PROJECT_NAME>AbilitySystemComponent* GetPlayerAbilitySystemComponent() const;
};
