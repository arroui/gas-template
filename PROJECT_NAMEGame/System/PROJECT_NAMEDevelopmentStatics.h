...
#include "<PROJECT_NAME>DevelopmentStatics.generated.h"

class UWorld;

UCLASS()
class U<PROJECT_NAME>DevelopmentStatics : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()

public:
	// Should game logic skip directly to gameplay (skipping any match warmup / waiting for players / etc... aspects)
	// Will always return false except when playing in the editor and bTestFullGameFlowInPIE (in <PROJECT_NAME> Developer Settings) is false
	...
	// Will always return true except when playing in the editor and bSkipLoadingCosmeticBackgroundsInPIE (in <PROJECT_NAME> Developer Settings) is true
	UFUNCTION(BlueprintCallable, Category = "<PROJECT_NAME>", meta=(ExpandBoolAsExecs="ReturnValue"))
