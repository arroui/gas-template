...
#include "<PROJECT_NAME>DevelopmentStatics.h"
#include "Development/<PROJECT_NAME>DeveloperSettings.h"
#include "Engine/Engine.h"
#include "AssetRegistryModule.h"

bool U<PROJECT_NAME>DevelopmentStatics::ShouldSkipDirectlyToGameplay()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return !GetDefault<U<PROJECT_NAME>DeveloperSettings>()->bTestFullGameFlowInPIE;
	}
#endif
	return false;
}

bool U<PROJECT_NAME>DevelopmentStatics::ShouldLoadCosmeticBackgrounds()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return !GetDefault<U<PROJECT_NAME>DeveloperSettings>()->bSkipLoadingCosmeticBackgroundsInPIE;
	}
#endif
	return true;
}

bool U<PROJECT_NAME>DevelopmentStatics::CanPlayerBotsAttack()
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		return GetDefault<U<PROJECT_NAME>DeveloperSettings>()->bAllowPlayerBotsToAttack;
	}
#endif
	return true;
}
...
UWorld* U<PROJECT_NAME>DevelopmentStatics::FindPlayInEditorAuthorityWorld()
{
	check(GEngine);
...
TArray<FAssetData> U<PROJECT_NAME>DevelopmentStatics::GetAllBlueprints()
{
...
UClass* U<PROJECT_NAME>DevelopmentStatics::FindBlueprintClass(const FString& TargetNameRaw, UClass* DesiredBaseClass)
{
	FString TargetName = TargetNameRaw;
	TargetName.RemoveFromEnd(TEXT("_C"), ESearchCase::CaseSensitive);

	TArray<FAssetData> BlueprintList = U<PROJECT_NAME>DevelopmentStatics::GetAllBlueprints();
...
UClass* U<PROJECT_NAME>DevelopmentStatics::FindClassByShortName(const FString& SearchToken, UClass* DesiredBaseClass, bool bLogFailures)
{
