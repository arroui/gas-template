...
#include "<PROJECT_NAME>CameraMode_ThirdPerson.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "<PROJECT_NAME>CameraAssistInterface.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"

namespace <PROJECT_NAME>CameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

U<PROJECT_NAME>CameraMode_ThirdPerson::U<PROJECT_NAME>CameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(F<PROJECT_NAME>PenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(F<PROJECT_NAME>PenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(F<PROJECT_NAME>PenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(F<PROJECT_NAME>PenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(F<PROJECT_NAME>PenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(F<PROJECT_NAME>PenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(F<PROJECT_NAME>PenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void U<PROJECT_NAME>CameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void U<PROJECT_NAME>CameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void U<PROJECT_NAME>CameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void U<PROJECT_NAME>CameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	I<PROJECT_NAME>CameraAssistInterface* TargetControllerAssist = Cast<I<PROJECT_NAME>CameraAssistInterface>(TargetController);

	I<PROJECT_NAME>CameraAssistInterface* TargetActorAssist = Cast<I<PROJECT_NAME>CameraAssistInterface>(TargetActor);
	...
	I<PROJECT_NAME>CameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<I<PROJECT_NAME>CameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		I<PROJECT_NAME>CameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (I<PROJECT_NAME>CameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void U<PROJECT_NAME>CameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO I<PROJECT_NAME>CameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		F<PROJECT_NAME>PenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(<PROJECT_NAME>CameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void U<PROJECT_NAME>CameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void U<PROJECT_NAME>CameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
