...
EActiveTimerReturnType SActorCanvas::UpdateCanvas(double InCurrentTime, float InDeltaTime)
{
	...
	U<PROJECT_NAME>IndicatorManagerComponent* IndicatorComponent = IndicatorComponentPtr.Get();
	if (IndicatorComponent == nullptr)
	{
		IndicatorComponent = U<PROJECT_NAME>IndicatorManagerComponent::GetComponent(LocalPlayerContext.GetPlayerController());
