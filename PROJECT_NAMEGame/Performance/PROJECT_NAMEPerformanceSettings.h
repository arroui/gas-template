...
#include "<PROJECT_NAME>PerformanceSettings.generated.h"

// Describes one platform-specific device profile variant that the user can choose from in the UI
USTRUCT()
struct F<PROJECT_NAME>QualityDeviceProfileVariant
{
	GENERATED_BODY()
...
USTRUCT()
struct F<PROJECT_NAME>PerformanceStatGroup
{
	GENERATED_BODY()
	...
	UPROPERTY(EditAnywhere)
	TSet<E<PROJECT_NAME>DisplayablePerformanceStat> AllowedStats;
};

// How hare frame pacing and overall graphics settings controlled/exposed for the platform?
UENUM()
enum class E<PROJECT_NAME>FramePacingMode : uint8
{
...
class U<PROJECT_NAME>PlatformSpecificRenderingSettings : public UPlatformSettings
{
	GENERATED_BODY()

public:
	U<PROJECT_NAME>PlatformSpecificRenderingSettings();

	// Helper method to get the performance settings object, directed via platform settings
	static const U<PROJECT_NAME>PlatformSpecificRenderingSettings* Get();
...
public:
	...
	// Note that this will usually be set from platform-specific ini files, not via the UI
	UPROPERTY(EditAnywhere, Config, Category=DeviceProfiles)
	TArray<F<PROJECT_NAME>QualityDeviceProfileVariant> UserFacingDeviceProfileOptions;
	...
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings)
	E<PROJECT_NAME>FramePacingMode FramePacingMode = E<PROJECT_NAME>FramePacingMode::DesktopStyle;

	// Potential frame rates to display for mobile
	// Note: This is further limited by <PROJECT_NAME>.DeviceProfile.Mobile.MaxFrameRate from the
	// platform-specific device profile and what the platform frame pacer reports as supported
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings, meta=(EditCondition="FramePacingMode==E<PROJECT_NAME>FramePacingMode::MobileStyle", ForceUnits=Hz))
 ...
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="<PROJECT_NAME> Performance Settings"))
class U<PROJECT_NAME>PerformanceSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	U<PROJECT_NAME>PerformanceSettings();
...
public:
	...
	TArray<F<PROJECT_NAME>PerformanceStatGroup> UserFacingPerformanceStats;
};