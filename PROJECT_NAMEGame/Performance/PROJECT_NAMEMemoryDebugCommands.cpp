...
FAutoConsoleCommandWithWorldAndArgs GObjListToCollectionCmd(
	TEXT("<PROJECT_NAME>.ObjListToCollection"),
		...
		[](const TArray<FString>& Params, UWorld* World)
{
	...
	UE_LOG(Log<PROJECT_NAME>, Warning, TEXT("Wrote collection of loaded assets to %s"), *CollectionFilePath);
}));
...
void AnalyzeObjectListForDifferences(TArrayView<UObject*> ObjectList, UClass* CommonClass, const TSet<FName>& PropertiesToIgnore, bool bLogAllMatchedDefault=false)
{
	...
	UE_LOG(Log<PROJECT_NAME>, Log, TEXT("  Field\tDifferentToBase\tNumValues\tValues"));
	...
	for (TFieldIterator<FProperty> PropIt(CommonClass); PropIt; ++PropIt)
	{
		...
		if (!bAllMatchedDefaultValue)
		{
			...
			UE_LOG(Log<PROJECT_NAME>, Log, TEXT("  %s::%s\t%s\t%d\t%s"),
				*CommonClass->GetName(),
				*Prop->GetName(),
		...
		{
			UE_LOG(Log<PROJECT_NAME>, Log, TEXT("  %s::%s\t%s"), *CommonClass->GetName(), *Prop->GetName(), TEXT("Default"));
		}
	}
}
