...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FTeamColorObservedAsyncDelegate, bool, bTeamSet, int32, TeamId, const U<PROJECT_NAME>TeamDisplayAsset*, DisplayAsset);
...
private:
	void BroadcastChange(int32 NewTeam, const U<PROJECT_NAME>TeamDisplayAsset* DisplayAsset);

	UFUNCTION()
	void OnWatchedAgentChangedTeam(UObject* TeamAgent, int32 OldTeam, int32 NewTeam);

	UFUNCTION()
	void OnDisplayAssetChanged(const U<PROJECT_NAME>TeamDisplayAsset* DisplayAsset);
