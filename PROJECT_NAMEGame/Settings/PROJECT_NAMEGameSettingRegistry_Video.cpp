...
#include "CustomSettings/<PROJECT_NAME>SettingAction_SafeZoneEditor.h"
#include "CustomSettings/<PROJECT_NAME>SettingValueDiscrete_Resolution.h"
#include "CustomSettings/<PROJECT_NAME>SettingValueDiscrete_OverallQuality.h"
#include "CustomSettings/<PROJECT_NAME>SettingValueDiscrete_MobileFPSType.h"
...
{
public:
	FGameSettingEditCondition_FramePacingMode(E<PROJECT_NAME>FramePacingMode InDesiredMode, EFramePacingEditCondition InMatchMode = EFramePacingEditCondition::EnableIf)
	...
	{
		const E<PROJECT_NAME>FramePacingMode ActualMode = U<PROJECT_NAME>PlatformSpecificRenderingSettings::Get()->FramePacingMode;
...
private:
	E<PROJECT_NAME>FramePacingMode DesiredMode;
...
public:
	...
	virtual void GatherEditState(const ULocalPlayer* InLocalPlayer, FGameSettingEditableState& InOutEditState) const override
	{
		if (!U<PROJECT_NAME>PlatformSpecificRenderingSettings::Get()->bSupportsGranularVideoQualitySettings)
		{
	...
	virtual void SettingChanged(const ULocalPlayer* LocalPlayer, UGameSetting* Setting, EGameSettingChangeReason Reason) const override
	{
		...
		const U<PROJECT_NAME>LocalPlayer* <PROJECT_NAME>LocalPlayer = CastChecked<U<PROJECT_NAME>LocalPlayer>(LocalPlayer);
		<PROJECT_NAME>LocalPlayer->GetLocalSettings()->ApplyScalabilitySettings();
	}
...
UGameSettingCollection* U<PROJECT_NAME>GameSettingRegistry::InitializeVideoSettings(U<PROJECT_NAME>LocalPlayer* InLocalPlayer)
{
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			U<PROJECT_NAME>SettingValueDiscrete_Resolution* Setting = NewObject<U<PROJECT_NAME>SettingValueDiscrete_Resolution>();
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<U<PROJECT_NAME>SettingsShared>()->GetColorBlindMode());
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<U<PROJECT_NAME>SettingsShared>()->GetColorBlindStrength());
		...
		//----------------------------------------------------------------------------------
		{
			U<PROJECT_NAME>SettingAction_SafeZoneEditor* Setting = NewObject<U<PROJECT_NAME>SettingAction_SafeZoneEditor>();
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			for (const F<PROJECT_NAME>QualityDeviceProfileVariant& Variant : PlatformSettings->UserFacingDeviceProfileOptions)
			{
		...
		//----------------------------------------------------------------------------------
		{
			...
			U<PROJECT_NAME>SettingValueDiscrete_MobileFPSType* Setting = NewObject<U<PROJECT_NAME>SettingValueDiscrete_MobileFPSType>();
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(E<PROJECT_NAME>FramePacingMode::MobileStyle));
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetCustomAction([](ULocalPlayer* LocalPlayer)
			{
				...
				{
					U<PROJECT_NAME>SettingsLocal::Get()->ResetToMobileDeviceDefaults();
				}
				else
				{
					const U<PROJECT_NAME>LocalPlayer* <PROJECT_NAME>LocalPlayer = CastChecked<U<PROJECT_NAME>LocalPlayer>(LocalPlayer);
					...
					<PROJECT_NAME>LocalPlayer->GetLocalSettings()->RunAutoBenchmark(bImmediatelySaveState);
				}
			});
			...
			{
				const U<PROJECT_NAME>PlatformSpecificRenderingSettings* PlatformSettings = U<PROJECT_NAME>PlatformSpecificRenderingSettings::Get();
				const bool bCanUseDueToMobile = (PlatformSettings->FramePacingMode == E<PROJECT_NAME>FramePacingMode::MobileStyle);

				const U<PROJECT_NAME>LocalPlayer* <PROJECT_NAME>LocalPlayer = CastChecked<U<PROJECT_NAME>LocalPlayer>(LocalPlayer);
				const bool bCanBenchmark = <PROJECT_NAME>LocalPlayer->GetLocalSettings()->CanRunAutoBenchmark();
		...
		//----------------------------------------------------------------------------------
		{
			U<PROJECT_NAME>SettingValueDiscrete_OverallQuality* Setting = NewObject<U<PROJECT_NAME>SettingValueDiscrete_OverallQuality>();
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(E<PROJECT_NAME>FramePacingMode::ConsoleStyle, EFramePacingEditCondition::DisableIf));
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(E<PROJECT_NAME>FramePacingMode::DesktopStyle));
...
void AddFrameRateOptions(UGameSettingValueDiscreteDynamic_Number* Setting)
{
	...
	for (int32 Rate : GetDefault<U<PROJECT_NAME>PerformanceSettings>()->DesktopFrameRateLimits)
	{
...
void U<PROJECT_NAME>GameSettingRegistry::InitializeVideoSettings_FrameRates(UGameSettingCollection* Screen, U<PROJECT_NAME>LocalPlayer* InLocalPlayer)
{
	//----------------------------------------------------------------------------------
	{
		UGameSettingValueDiscreteDynamic_Number* Setting = NewObject<UGameSettingValueDiscreteDynamic_Number>();
		...
		Setting->SetDefaultValue(GetDefault<U<PROJECT_NAME>SettingsLocal>()->GetFrameRateLimit_OnBattery());
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<U<PROJECT_NAME>SettingsLocal>()->GetFrameRateLimit_InMenu());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(E<PROJECT_NAME>FramePacingMode::DesktopStyle));
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<U<PROJECT_NAME>SettingsLocal>()->GetFrameRateLimit_WhenBackgrounded());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(E<PROJECT_NAME>FramePacingMode::DesktopStyle));
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<U<PROJECT_NAME>SettingsLocal>()->GetFrameRateLimit_Always());
		Setting->AddEditCondition(MakeShared<FGameSettingEditCondition_FramePacingMode>(E<PROJECT_NAME>FramePacingMode::DesktopStyle));
