...
#include "<PROJECT_NAME>SettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class E<PROJECT_NAME>AllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
UENUM(BlueprintType)
enum class E<PROJECT_NAME>GamepadSensitivity : uint8
{
...
/**
 * U<PROJECT_NAME>SettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 *               maybe change Localsettings to <PROJECT_NAME>PlatformSettings, or DeviceSettings?  Make this one UserSettings?  TBD
 *               
...
class U<PROJECT_NAME>SettingsShared : public USaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(U<PROJECT_NAME>SettingsShared, FOnSettingChangedEvent, U<PROJECT_NAME>SettingsShared* Settings);
	FOnSettingChangedEvent OnSettingChanged;

public:

	U<PROJECT_NAME>SettingsShared();

	void Initialize(U<PROJECT_NAME>LocalPlayer* LocalPlayer);

	bool IsDirty() const { return bIsDirty; }
	void ClearDirtyFlag() { bIsDirty = false; }

	void SaveSettings();
	static U<PROJECT_NAME>SettingsShared* LoadOrCreateSettings(const U<PROJECT_NAME>LocalPlayer* LocalPlayer);
...
public:
	UFUNCTION()
	E<PROJECT_NAME>AllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(E<PROJECT_NAME>AllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	E<PROJECT_NAME>AllowBackgroundAudioSetting AllowAudioInBackground = E<PROJECT_NAME>AllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	E<PROJECT_NAME>GamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(E<PROJECT_NAME>GamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	E<PROJECT_NAME>GamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(E<PROJECT_NAME>GamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	E<PROJECT_NAME>GamepadSensitivity GamepadLookSensitivityPreset = E<PROJECT_NAME>GamepadSensitivity::Normal;
	UPROPERTY()
	E<PROJECT_NAME>GamepadSensitivity GamepadTargetingSensitivityPreset = E<PROJECT_NAME>GamepadSensitivity::Normal;
...
private:
	...
	UPROPERTY(Transient)
	U<PROJECT_NAME>LocalPlayer* OwningPlayer = nullptr;
};
