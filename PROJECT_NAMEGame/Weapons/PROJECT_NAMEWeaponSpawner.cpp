...
#include "<PROJECT_NAME>WeaponSpawner.h"
...
#include "Equipment/<PROJECT_NAME>PickupDefinition.h"
...
// Sets default values
A<PROJECT_NAME>WeaponSpawner::A<PROJECT_NAME>WeaponSpawner()
{
	...
	CollisionVolume->OnComponentBeginOverlap.AddDynamic(this, &A<PROJECT_NAME>WeaponSpawner::OnOverlapBegin);
...
void A<PROJECT_NAME>WeaponSpawner::BeginPlay()
{
	Super::BeginPlay();
	...
	{
		UE_LOG(Log<PROJECT_NAME>, Error, TEXT("'%s' does not have a valid weapon definition! Make sure to set this data on the instance!"), *GetNameSafe(this));
	}
}

void A<PROJECT_NAME>WeaponSpawner::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
// Called every frame
void A<PROJECT_NAME>WeaponSpawner::Tick(float DeltaTime)
{
...
void A<PROJECT_NAME>WeaponSpawner::OnConstruction(const FTransform& Transform)
{
...
void A<PROJECT_NAME>WeaponSpawner::OnOverlapBegin(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepHitResult)
{
...
void A<PROJECT_NAME>WeaponSpawner::CheckForExistingOverlaps()
{
...
void A<PROJECT_NAME>WeaponSpawner::AttemptPickUpWeapon_Implementation(APawn* Pawn)
{
	if (GetLocalRole() == ROLE_Authority && bIsWeaponAvailable && UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(Pawn))
	{
		TSubclassOf<U<PROJECT_NAME>InventoryItemDefinition> WeaponItemDefinition = WeaponDefinition ? WeaponDefinition->InventoryItemDefinition : nullptr;
...
void A<PROJECT_NAME>WeaponSpawner::StartCoolDown()
{
	if (UWorld* World = GetWorld())
	{
		World->GetTimerManager().SetTimer(CoolDownTimerHandle, this, &A<PROJECT_NAME>WeaponSpawner::OnCoolDownTimerComplete, CoolDownTime);
	}
}

void A<PROJECT_NAME>WeaponSpawner::ResetCoolDown()
{
	UWorld* World = GetWorld();
	...
	if (GetLocalRole() == ROLE_Authority)
	{
		...
		if (World)
		{
			World->GetTimerManager().SetTimer(CheckOverlapsDelayTimerHandle, this, &A<PROJECT_NAME>WeaponSpawner::CheckForExistingOverlaps, CheckExistingOverlapDelay);
		}
	}
...
void A<PROJECT_NAME>WeaponSpawner::OnCoolDownTimerComplete()
{
	ResetCoolDown();
}

void A<PROJECT_NAME>WeaponSpawner::SetWeaponPickupVisibility(bool bShouldBeVisible)
{
	WeaponMesh->SetVisibility(bShouldBeVisible, true);
}

void A<PROJECT_NAME>WeaponSpawner::PlayPickupEffects_Implementation()
{
...
void A<PROJECT_NAME>WeaponSpawner::PlayRespawnEffects_Implementation()
{
	if (WeaponDefinition != nullptr)
	{
...
void A<PROJECT_NAME>WeaponSpawner::OnRep_WeaponAvailability()
{
...
void A<PROJECT_NAME>WeaponSpawner::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(A<PROJECT_NAME>WeaponSpawner, bIsWeaponAvailable);
}

int32 A<PROJECT_NAME>WeaponSpawner::GetDefaultStatFromItemDef(const TSubclassOf<U<PROJECT_NAME>InventoryItemDefinition> WeaponItemClass, FGameplayTag StatTag)
{
	if (WeaponItemClass != nullptr)
	{
		if (U<PROJECT_NAME>InventoryItemDefinition* WeaponItemCDO = WeaponItemClass->GetDefaultObject<U<PROJECT_NAME>InventoryItemDefinition>())
		{
