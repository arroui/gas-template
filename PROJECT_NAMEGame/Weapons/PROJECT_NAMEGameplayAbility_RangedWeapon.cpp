...
U<PROJECT_NAME>GameplayAbility_RangedWeapon::U<PROJECT_NAME>GameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

U<PROJECT_NAME>RangedWeaponInstance* U<PROJECT_NAME>GameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<U<PROJECT_NAME>RangedWeaponInstance>(GetAssociatedEquipment());
}

bool U<PROJECT_NAME>GameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(Log<PROJECT_NAME>AbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*U<PROJECT_NAME>RangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 U<PROJECT_NAME>GameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void U<PROJECT_NAME>GameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel U<PROJECT_NAME>GameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return <PROJECT_NAME>_TraceChannel_Weapon;
}

FHitResult U<PROJECT_NAME>GameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector U<PROJECT_NAME>GameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform U<PROJECT_NAME>GameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, E<PROJECT_NAME>AbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	U<PROJECT_NAME>WeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<U<PROJECT_NAME>WeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != E<PROJECT_NAME>AbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == E<PROJECT_NAME>AbilityTargetingSource::CameraTowardsFocus) || (Source == E<PROJECT_NAME>AbilityTargetingSource::PawnTowardsFocus) || (Source == E<PROJECT_NAME>AbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == E<PROJECT_NAME>AbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == E<PROJECT_NAME>AbilityTargetingSource::WeaponForward) || (Source == E<PROJECT_NAME>AbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == E<PROJECT_NAME>AbilityTargetingSource::PawnTowardsFocus) || (Source == E<PROJECT_NAME>AbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult U<PROJECT_NAME>GameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (<PROJECT_NAME>ConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, <PROJECT_NAME>ConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void U<PROJECT_NAME>GameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	U<PROJECT_NAME>RangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, E<PROJECT_NAME>AbilityTargetingSource::CameraTowardsFocus);
		...
		if (<PROJECT_NAME>ConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, <PROJECT_NAME>ConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void U<PROJECT_NAME>GameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	U<PROJECT_NAME>RangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (<PROJECT_NAME>ConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, <PROJECT_NAME>ConsoleVariables::DrawBulletHitRadius, FColor::Red, false, <PROJECT_NAME>ConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void U<PROJECT_NAME>GameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void U<PROJECT_NAME>GameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void U<PROJECT_NAME>GameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (U<PROJECT_NAME>WeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<U<PROJECT_NAME>WeaponStateComponent>())
					{
		...
		{
			UE_LOG(Log<PROJECT_NAME>AbilitySystem, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void U<PROJECT_NAME>GameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	U<PROJECT_NAME>WeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<U<PROJECT_NAME>WeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			F<PROJECT_NAME>GameplayAbilityTargetData_SingleTargetHit* NewTargetData = new F<PROJECT_NAME>GameplayAbilityTargetData_SingleTargetHit();
