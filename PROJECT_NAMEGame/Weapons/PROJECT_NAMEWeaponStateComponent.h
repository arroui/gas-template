...
#include "<PROJECT_NAME>WeaponStateComponent.generated.h"
...
struct F<PROJECT_NAME>ScreenSpaceHitLocation
{
...
struct F<PROJECT_NAME>ServerSideHitMarkerBatch
{
	F<PROJECT_NAME>ServerSideHitMarkerBatch() { }

	F<PROJECT_NAME>ServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<F<PROJECT_NAME>ScreenSpaceHitLocation> Markers;
...
UCLASS()
class U<PROJECT_NAME>WeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	U<PROJECT_NAME>WeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<F<PROJECT_NAME>ScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<F<PROJECT_NAME>ScreenSpaceHitLocation> LastWeaponDamageScreenLocations;

	/** The unconfirmed hits */
	TArray<F<PROJECT_NAME>ServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
};
