...
#include "<PROJECT_NAME>RangedWeaponInstance.h"
...
UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_<PROJECT_NAME>_Weapon_SteadyAimingCamera, "<PROJECT_NAME>.Weapon.SteadyAimingCamera");

U<PROJECT_NAME>RangedWeaponInstance::U<PROJECT_NAME>RangedWeaponInstance(const FObjectInitializer& ObjectInitializer)
...
void U<PROJECT_NAME>RangedWeaponInstance::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void U<PROJECT_NAME>RangedWeaponInstance::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	UpdateDebugVisualization();
}

void U<PROJECT_NAME>RangedWeaponInstance::UpdateDebugVisualization()
{
...
void U<PROJECT_NAME>RangedWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void U<PROJECT_NAME>RangedWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
}

void U<PROJECT_NAME>RangedWeaponInstance::Tick(float DeltaSeconds)
{
	APawn* Pawn = GetPawn();
	check(Pawn != nullptr);
...
void U<PROJECT_NAME>RangedWeaponInstance::ComputeHeatRange(float& MinHeat, float& MaxHeat)
{
	float Min1;
	float Max1;
...
void U<PROJECT_NAME>RangedWeaponInstance::ComputeSpreadRange(float& MinSpread, float& MaxSpread)
{
	HeatToSpreadCurve.GetRichCurveConst()->GetValueRange(/*out*/ MinSpread, /*out*/ MaxSpread);
}

void U<PROJECT_NAME>RangedWeaponInstance::AddSpread()
{
...
float U<PROJECT_NAME>RangedWeaponInstance::GetDistanceAttenuation(float Distance, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
	const FRichCurve* Curve = DistanceDamageFalloff.GetRichCurveConst();
	return Curve->HasAnyData() ? Curve->Eval(Distance) : 1.0f;
}

float U<PROJECT_NAME>RangedWeaponInstance::GetPhysicalMaterialAttenuation(const UPhysicalMaterial* PhysicalMaterial, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
...
bool U<PROJECT_NAME>RangedWeaponInstance::UpdateSpread(float DeltaSeconds)
{
...
bool U<PROJECT_NAME>RangedWeaponInstance::UpdateMultipliers(float DeltaSeconds)
{
	...
	if (const U<PROJECT_NAME>CameraComponent* CameraComponent = U<PROJECT_NAME>CameraComponent::FindCameraComponent(Pawn))
	{
		...
		AimingAlpha = (TopCameraTag == TAG_<PROJECT_NAME>_Weapon_SteadyAimingCamera) ? TopCameraWeight : 0.0f;
	}
