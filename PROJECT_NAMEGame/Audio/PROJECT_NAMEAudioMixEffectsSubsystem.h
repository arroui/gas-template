...
#include "<PROJECT_NAME>AudioMixEffectsSubsystem.generated.h"
...
USTRUCT()
struct F<PROJECT_NAME>AudioSubmixEffectsChain
{
	GENERATED_BODY()
 ...
 * <PROJECT_NAME> Audio Settings.
 */
UCLASS()
class <PROJECT_NAME_UPPER>GAME_API U<PROJECT_NAME>AudioMixEffectsSubsystem : public UWorldSubsystem
{
	GENERATED_BODY()
...
protected:
	...
	// Default Sound Control Bus Mix retrieved from the <PROJECT_NAME> Audio Settings
	UPROPERTY(Transient)
	USoundControlBusMix* DefaultBaseMix = nullptr;

	// Loading Screen Sound Control Bus Mix retrieved from the <PROJECT_NAME> Audio Settings
	UPROPERTY(Transient)
	USoundControlBusMix* LoadingScreenMix = nullptr;

	// User Sound Control Bus Mix retrieved from the <PROJECT_NAME> Audio Settings
	UPROPERTY(Transient)
	USoundControlBusMix* UserMix = nullptr;

	// Overall Sound Control Bus retrieved from the <PROJECT_NAME> Audio Settings and linked to the UI and game settings in <PROJECT_NAME>SettingsLocal
	UPROPERTY(Transient)
	USoundControlBus* OverallControlBus = nullptr;

	// Music Sound Control Bus retrieved from the <PROJECT_NAME> Audio Settings and linked to the UI and game settings in <PROJECT_NAME>SettingsLocal
	UPROPERTY(Transient)
	USoundControlBus* MusicControlBus = nullptr;

	// SoundFX Sound Control Bus retrieved from the <PROJECT_NAME> Audio Settings and linked to the UI and game settings in <PROJECT_NAME>SettingsLocal
	UPROPERTY(Transient)
	USoundControlBus* SoundFXControlBus = nullptr;

	// Dialogue Sound Control Bus retrieved from the <PROJECT_NAME> Audio Settings and linked to the UI and game settings in <PROJECT_NAME>SettingsLocal
	UPROPERTY(Transient)
	USoundControlBus* DialogueControlBus = nullptr;

	// VoiceChat Sound Control Bus retrieved from the <PROJECT_NAME> Audio Settings and linked to the UI and game settings in <PROJECT_NAME>SettingsLocal
	UPROPERTY(Transient)
	USoundControlBus* VoiceChatControlBus = nullptr;

	// Submix Effect Chain Overrides to apply when HDR Audio is turned on
	UPROPERTY(Transient)
	TArray<F<PROJECT_NAME>AudioSubmixEffectsChain> HDRSubmixEffectChain;

	// Submix Effect hain Overrides to apply when HDR Audio is turned off
	UPROPERTY(Transient)
	TArray<F<PROJECT_NAME>AudioSubmixEffectsChain> LDRSubmixEffectChain;
