...
#include "<PROJECT_NAME>AudioSettings.h"
#include "Settings/<PROJECT_NAME>SettingsLocal.h"
#include "AudioMixerBlueprintLibrary.h"
#include "LoadingScreenManager.h"
#include "Engine/GameInstance.h"

void U<PROJECT_NAME>AudioMixEffectsSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);
}

void U<PROJECT_NAME>AudioMixEffectsSubsystem::Deinitialize()
{
...
bool U<PROJECT_NAME>AudioMixEffectsSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
...
void U<PROJECT_NAME>AudioMixEffectsSubsystem::PostInitialize()
{
	if (const U<PROJECT_NAME>AudioSettings* <PROJECT_NAME>AudioSettings = GetDefault<U<PROJECT_NAME>AudioSettings>())
	{
		if (UObject* ObjPath = <PROJECT_NAME>AudioSettings->DefaultControlBusMix.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Default Control Bus Mix reference missing from <PROJECT_NAME> Audio Settings."));
			}
		}

		if (UObject* ObjPath = <PROJECT_NAME>AudioSettings->LoadingScreenControlBusMix.TryLoad())
		{
			if (USoundControlBusMix* SoundControlBusMix = Cast<USoundControlBusMix>(ObjPath))
			{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Loading Screen Control Bus Mix reference missing from <PROJECT_NAME> Audio Settings."));
			}
		}
		...
		if (UObject* ObjPath = <PROJECT_NAME>AudioSettings->UserSettingsControlBusMix.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("User Control Bus Mix reference missing from <PROJECT_NAME> Audio Settings."));
			}
		}
		...
		for (const F<PROJECT_NAME>SubmixEffectChainMap& SoftSubmixEffectChain : <PROJECT_NAME>AudioSettings->HDRAudioSubmixEffectChain)
		{
			F<PROJECT_NAME>AudioSubmixEffectsChain NewEffectChain;
		...
		for (const F<PROJECT_NAME>SubmixEffectChainMap& SoftSubmixEffectChain : <PROJECT_NAME>AudioSettings->LDRAudioSubmixEffectChain)
		{
			F<PROJECT_NAME>AudioSubmixEffectsChain NewEffectChain;
...
void U<PROJECT_NAME>AudioMixEffectsSubsystem::OnWorldBeginPlay(UWorld& InWorld)
{
	if (const UWorld* World = InWorld.GetWorld())
	{
		...
		if (const U<PROJECT_NAME>SettingsLocal* <PROJECT_NAME>SettingsLocal = GetDefault<U<PROJECT_NAME>SettingsLocal>())
		{
			...
			if (UserMix)
			{
				...
				{
					const FSoundControlBusMixStage OverallControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, OverallControlBus, <PROJECT_NAME>SettingsLocal->GetOverallVolume());
					const FSoundControlBusMixStage MusicControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, MusicControlBus, <PROJECT_NAME>SettingsLocal->GetMusicVolume());
					const FSoundControlBusMixStage SoundFXControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, SoundFXControlBus, <PROJECT_NAME>SettingsLocal->GetSoundFXVolume());
					const FSoundControlBusMixStage DialogueControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, DialogueControlBus, <PROJECT_NAME>SettingsLocal->GetDialogueVolume());
					const FSoundControlBusMixStage VoiceChatControlBusMixStage = UAudioModulationStatics::CreateBusMixStage(World, VoiceChatControlBus, <PROJECT_NAME>SettingsLocal->GetVoiceChatVolume());
			...
			ApplyDynamicRangeEffectsChains(<PROJECT_NAME>SettingsLocal->IsHDRAudioModeEnabled());
		}
	}
}

void U<PROJECT_NAME>AudioMixEffectsSubsystem::ApplyDynamicRangeEffectsChains(bool bHDRAudio)
{
	TArray<F<PROJECT_NAME>AudioSubmixEffectsChain> AudioSubmixEffectsChainToApply;
	TArray<F<PROJECT_NAME>AudioSubmixEffectsChain> AudioSubmixEffectsChainToClear;
	...
	for (const F<PROJECT_NAME>AudioSubmixEffectsChain& EffectChainToClear : AudioSubmixEffectsChainToClear)
	{
		bool bAddToList = true;

		for (const F<PROJECT_NAME>AudioSubmixEffectsChain& SubmixEffectChain : AudioSubmixEffectsChainToApply)
		{
...
void U<PROJECT_NAME>AudioMixEffectsSubsystem::OnLoadingScreenStatusChanged(bool bShowingLoadingScreen)
{
	ApplyOrRemoveLoadingScreenMix(bShowingLoadingScreen);
}

void U<PROJECT_NAME>AudioMixEffectsSubsystem::ApplyOrRemoveLoadingScreenMix(bool bWantsLoadingScreenMix)
{
...
bool U<PROJECT_NAME>AudioMixEffectsSubsystem::DoesSupportWorldType(const EWorldType::Type World) const
{
