...
#include "<PROJECT_NAME>InputModifiers.generated.h"

class UEnhancedPlayerInput;
class U<PROJECT_NAME>AimSensitivityData;
...
class U<PROJECT_NAME>SettingBasedScalar : public UInputModifier
{
	GENERATED_BODY()
...
/**
 * This is a deadzone input modifier that will have it's thresholds driven by what is in the <PROJECT_NAME> Shared game settings. 
 */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "<PROJECT_NAME> Settings Driven Dead Zone"))
class U<PROJECT_NAME>InputModifierDeadZone : public UInputModifier
{
	GENERATED_BODY()
...
UENUM()
enum class E<PROJECT_NAME>TargetingType : uint8
{
...
/** Applies a scalar modifier based on the current gamepad settings in <PROJECT_NAME> Shared game settings.  */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "<PROJECT_NAME> Gamepad Sensitivity"))
class U<PROJECT_NAME>InputModifierGamepadSensitivity : public UInputModifier
{
	GENERATED_BODY()
public:
	...
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category=Settings, Config)
	E<PROJECT_NAME>TargetingType TargetingType = E<PROJECT_NAME>TargetingType::Normal;

	/** Asset that gives us access to the float scalar value being used for sensitivty */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, meta=(AssetBundles="Client,Server"))
	TObjectPtr<const U<PROJECT_NAME>AimSensitivityData> SensitivityLevelTable;

protected:
	virtual FInputActionValue ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime) override;
};

/** Applies an inversion of axis values based on a setting in the <PROJECT_NAME> Shared game settings */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "<PROJECT_NAME> Aim Inversion Setting"))
class U<PROJECT_NAME>InputModifierAimInversion : public UInputModifier
{
	GENERATED_BODY()
