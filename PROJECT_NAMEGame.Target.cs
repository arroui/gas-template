...
public class <PROJECT_NAME>GameTarget : TargetRules
{
	public <PROJECT_NAME>GameTarget(TargetInfo Target) : base(Target)
	{
	...
	internal static void ApplyShared<PROJECT_NAME>TargetSettings(TargetRules Target)
    {
		...
		if (Target.BuildEnvironment == TargetBuildEnvironment.Unique)
		{
			...
			<PROJECT_NAME>GameTarget.ConfigureGameFeaturePlugins(Target);
		}
		else
        {
			// !!!!!!!!!!!! WARNING !!!!!!!!!!!!!
			...
			else
			{
				...
				if (!bHasWarnedAboutShared)
				{
					...
					Log.TraceWarning("<PROJECT_NAME>GameEOS and dynamic target options are disabled when packaging from an installed version of the engine");
				}
			}
		}		
	}
