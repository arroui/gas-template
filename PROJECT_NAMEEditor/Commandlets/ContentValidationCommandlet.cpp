...
DEFINE_LOG_CATEGORY_STATIC(Log<PROJECT_NAME>ContentValidation, Log, Log);
...
{
	UE_LOG(Log<PROJECT_NAME>ContentValidation, Display, TEXT("Running ContentValidationCommandlet commandlet..."));
	...
	if (P4FilterString && !P4FilterString->IsEmpty())
	{
		...
		{
			UE_LOG(Log<PROJECT_NAME>ContentValidation, Display, TEXT("ContentValidation returning 1. Failed to get changed files."));
			ReturnVal = 1;
		}
	}
	...
	if (bP4Opened)
	{
		...
		{
			UE_LOG(Log<PROJECT_NAME>ContentValidation, Error, TEXT("P4 workspace was not found when using P4Opened"));
			UE_LOG(Log<PROJECT_NAME>ContentValidation, Display, TEXT("ContentValidation returning 1. Workspace not found."));
			ReturnVal = 1;
		}
	}
...
bool UContentValidationCommandlet::GetAllChangedFiles(IAssetRegistry& AssetRegistry, const FString& P4CmdString, TArray<FString>& OutChangedPackageNames, TArray<FString>& DeletedPackageNames, TArray<FString>& OutChangedCode, TArray<FString>& OutChangedOtherFiles) const
{
	...
	if (LaunchP4(P4CmdString, Results, ReturnCode))
	{
				...
				if (Result.Split(TEXT("#"), &DepotPathName, &ExtraInfoAfterPound))
				{
					if (DepotPathName.EndsWith(TEXT(".uasset")) || DepotPathName.EndsWith(TEXT(".umap")))
					{
						FString FullPackageName;
						{
							...
							if (DepotPathName.Split(TEXT("<PROJECT_NAME>Game/Content/"), nullptr, &PostContentPath)) //@TODO: RENAME: Potential issue when modules are renamed
							{
						...
						if (FullPackageName.IsEmpty())
						{
							...
							if (DepotPathName.Split(TEXT("<PROJECT_NAME>Game/Plugins/"), nullptr, &PostPluginsPath))
							{
					...
					{
						FString Post<PROJECT_NAME>GamePath;
						if (DepotPathName.Split(TEXT("/<PROJECT_NAME>Game/"), nullptr, &Post<PROJECT_NAME>GamePath))
						{
							if (DepotPathName.EndsWith(TEXT(".cpp")))
							{
								OutChangedCode.Add(Post<PROJECT_NAME>GamePath);
							}
							...
							{
								OutChangedOtherFiles.Add(Post<PROJECT_NAME>GamePath);
							}
						}
					}
				}
		...
		{
			UE_LOG(Log<PROJECT_NAME>ContentValidation, Error, TEXT("p4 returned non-zero return code %d"), ReturnCode);
		}
	}
...
bool UContentValidationCommandlet::LaunchP4(const FString& Args, TArray<FString>& Output, int32& OutReturnCode) const
{
	...
	{
		UE_LOG(Log<PROJECT_NAME>ContentValidation, Error, TEXT("Failed to launch p4."));
	}
...
FString UContentValidationCommandlet::GetLocalPathFromDepotPath(const FString& DepotPathName) const
{
	...
	if (!Workspace.IsEmpty())
	{
		...
		if (LaunchP4(P4WhereCommand, WhereResults, ReturnCode))
		{
			...
			{
				UE_LOG(Log<PROJECT_NAME>ContentValidation, Warning, TEXT("GetAllChangedFiles failed to run p4 'where'. WhereResults[0] = '%s'. Not adding any validation for %s"), WhereResults.Num() > 0 ? *WhereResults[0] : TEXT("Invalid"), *DepotPathName);
			}
		}
	}
