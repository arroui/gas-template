...
#include "Private/AssetTypeActions_<PROJECT_NAME>ContextEffectsLibrary.h"
...
DEFINE_LOG_CATEGORY(Log<PROJECT_NAME>Editor);
...
/**
 * F<PROJECT_NAME>EditorModule
 */
class F<PROJECT_NAME>EditorModule : public FDefaultGameModuleImpl
{
	typedef F<PROJECT_NAME>EditorModule ThisClass;
	...
	virtual void StartupModule() override
	{
		...
		if (!IsRunningGame())
		{
			FModuleManager::Get().OnModulesChanged().AddRaw(this, &F<PROJECT_NAME>EditorModule::ModulesChangedCallback);
		...
		AssetTools.RegisterAssetTypeActions(MakeShareable(new FAssetTypeActions_<PROJECT_NAME>ContextEffectsLibrary));
	}

	void OnBeginPIE(bool bIsSimulating)
	{
		U<PROJECT_NAME>ExperienceManager* ExperienceManager = GEngine->GetEngineSubsystem<U<PROJECT_NAME>ExperienceManager>();
...
IMPLEMENT_MODULE(F<PROJECT_NAME>EditorModule, <PROJECT_NAME>Editor);
