...
TArray<FString> F<PROJECT_NAME>ValidationMessageGatherer::IgnorePatterns;
...
void UEditorValidator::ValidateCheckedOutContent(bool bInteractive, const EDataValidationUsecase InValidationUsecase)
{
	...
	if (AssetRegistryModule.Get().IsLoadingAssets())
	{
		...
		{
			UE_LOG(Log<PROJECT_NAME>Editor, Display, TEXT("Could not run ValidateCheckedOutContent because asset discovery was still being done."));
		}
		return;
	}
	...
	{
		F<PROJECT_NAME>ValidationMessageGatherer ScopedMessageGatherer;
...
bool UEditorValidator::ValidatePackages(const TArray<FString>& ExistingPackageNames, const TArray<FString>& DeletedPackageNames, int32 MaxPackagesToLoad, TArray<FString>& OutAllWarningsAndErrors, const EDataValidationUsecase InValidationUsecase)
{
	...
	{
		UE_LOG(Log<PROJECT_NAME>Editor, Display, TEXT("Adding referencers for deleted package %s to be verified"), *DeletedPackageName);
		...
		for (const FName& Referencer : PackageReferencers)
		{
			...
			{
				UE_LOG(Log<PROJECT_NAME>Editor, Display, TEXT("    Deleted package referencer %s was added to the queue to be verified"), *ReferencerString);
	...
	if (AllPackagesToValidate.Num() > MaxPackagesToLoad)
	{
		...
		UE_LOG(Log<PROJECT_NAME>Editor, Warning, TEXT("%s"), *WarningMessage);
		...
		if (AssetsToCheck.Num() > 0)
		{
			// Preload all assets to check, so load warnings can be handled separately from validation warnings
			{
				for (const FAssetData& AssetToCheck : AssetsToCheck)
				{
					...
					{
						UE_LOG(Log<PROJECT_NAME>Editor, Display, TEXT("Preloading %s..."), *AssetToCheck.ObjectPath.ToString());

						// Start listening for load warnings
						F<PROJECT_NAME>ValidationMessageGatherer ScopedPreloadMessageGatherer;
						...
						if (ScopedPreloadMessageGatherer.GetAllWarningsAndErrors().Num() > 0)
						{
							...
							{
								UE_LOG(Log<PROJECT_NAME>Editor, Error, TEXT("%s"), *LoadWarning);
							}
...
bool UEditorValidator::ValidateProjectSettings()
{
...

	{
		...
		if (bDeveloperMode)
		{
			...
			UE_LOG(Log<PROJECT_NAME>Editor, Error, TEXT("%s"), *ErrorMessage);
	...
	if (Module)
	{
		...
		for (TWeakObjectPtr<UClass> ModifiedClassPtr : ClassList)
		{
			...
			if (UClass* ModifiedClass = ModifiedClassPtr.Get())
			{
				...
				UE_LOG(Log<PROJECT_NAME>Editor, Display, TEXT("Validating Subclasses of %s in %s + %s"), *ModifiedClass->GetName(), *ChangedHeaderModule, *ChangedHeaderReleativeToModule);
				...
				AssetRegistry.EnumerateAssets(Filter, [&BlueprintsDerivedFromNativeModifiedClasses, &bTooManyFiles, &DerivedClassNames, ChangedHeaderModule, ChangedHeaderReleativeToModule](const FAssetData& AssetData)
					{
						...
						if (!AssetData.GetTagValueRef<bool>(FBlueprintTags::IsDataOnly) && !UEditorValidator::IsInUncookedFolder(PackageName))
						{
							...
							if (!ClassFromData.IsEmpty())
							{
								...
								{
									UE_LOG(Log<PROJECT_NAME>Editor, Display, TEXT("\tAdding %s To Validate"), *PackageName);
									...
									{
										bTooManyFiles = true;
										UE_LOG(Log<PROJECT_NAME>Editor, Display, TEXT("Too many assets invalidated (Max %d) by change to, %s + %s"), GMaxAssetsChangedByAHeader, *ChangedHeaderModule, *ChangedHeaderReleativeToModule);
